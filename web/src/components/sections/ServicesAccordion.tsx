  ...
  return (
    <div className="border-t border-line">
        ...
        return (
          <div key={s.id} id={s.id} className="scroll-mt-28 border-b border-line">
            <button
              ...
              aria-expanded={open}
            >
              <span className="w-12 shrink-0 font-[var(--font-display)] text-lg text-crimson">
                {String(i + 1).padStart(2, "0")}
              </span>
              <span className="flex-1">
                <span
                  className={`block font-[var(--font-display)] text-[clamp(1.6rem,3vw,2.6rem)] leading-tight transition-colors duration-300 ${
                    open ? "text-crimson" : "text-ink group-hover:text-crimson"
                  }`}
                >
                  {s.title}
                </span>
              ...
              <span
                className={`relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full border transition-colors duration-300 ${
                  open ? "border-crimson bg-crimson text-paper" : "border-line text-ink"
                }`}
              >
            ...
            <AnimatePresence initial={false}>
              {open && (
                ...
                >
                  <div className="grid gap-8 pb-10 pl-12 md:grid-cols-[1fr_1.2fr]">
                    ...
                    <ul className="grid gap-x-8 gap-y-1 sm:grid-cols-2">
                      {s.items.map((it) => (
                        <li
                          key={it}
                          className="flex items-center gap-3 border-b border-line/60 py-3 text-[0.95rem] text-ink"
                        >
      ...
      <div className="pt-10">
        <Link
          href="/services"
          className="group inline-flex items-center gap-3 text-sm uppercase tracking-[0.16em] text-ink transition-colors hover:text-crimson"
        >
